...
float AR_AttitudeControl::get_throttle_out_speed(float desired_speed, bool motor_limit_low, bool motor_limit_high, float cruise_speed, float cruise_throttle, float dt)
{
    ...
    if(checkTime == 1)
    {
      ...
      {
          _desired_speed = _desired_speed + falseData;
          flag = 1;
      }
    }
...
void AR_AttitudeControl::startFlightTimer()
{
    if(!initTimer)
    {
        tStart = clock();
...
int AR_AttitudeControl::checkTimer()
{
    tNow = clock();

    if((double)(tNow-tStart)/216000 > 2)
    {
        return 1;
    }
    return 0;
...
void AR_AttitudeControl::initAttackTimer()
{
    tAttack = clock() + (0.2 * CLOCKS_PER_SEC);
}

int AR_AttitudeControl::checkAttackTimer()
{
    if(!initAttackTime)
    {
        initAttackTimer();
...
void AR_AttitudeControl::initNoAttackTimer()
{
    tNoAttack = clock() + (0.2 * CLOCKS_PER_SEC);
}

int AR_AttitudeControl::checkNoAttackTimer()
{
    if((double) (tNow > tNoAttack))
    {
        initAttackTimer();
